...
public class MyGreenfootImage extends GreenfootImage
{
    ...
    public void shadow()
    {
        ...
        fillOval((int) (getWidth() * 0.1), (int) (3.5 * copy.getHeight() / 4.5), (int) (getWidth() * 0.8), getHeight() / 5);
        drawImage(copy, 0, 0);
    }
    ...
    public void highlight(Color highlight)
    {
        ...
        for(int x = 0; x < getWidth(); x++)
        {
            for(int y = 0; y < getHeight(); y++)
            {
                ...
                setColorAt(x, y, new Color(highlight.getRed(), highlight.getGreen(), highlight.getBlue(), colorHere.getAlpha()));
            }
        }
    ...
    private void applyHue(float hueEquipo)
    {
        for(int x = 0; x < getWidth(); x++)
        {
            for(int y = 0; y < getHeight(); y++)
            {
                ...
                setColorAt(x, y, new Color(colorEquipoHue.getRed(), colorEquipoHue.getGreen(), colorEquipoHue.getBlue(), color.getAlpha()));
            }
        }
    }
}